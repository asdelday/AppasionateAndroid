...
var alumnosLoaded = false, clasesLoaded = false;
var perfil = null;
var footables = {marcadores: null, alumnos: null, notas: null, cursos: null};
            ...
            case "alumnos":
                if (actual !== opcion) {
                    $('#main-' + actual).slideUp('400', function() {
                        ...
                        if (footables.alumnos == null) {
                            footables.alumnos = new FootableAlumnos();
                        } else {
                            footables.alumnos.getNotas();
                        }
...
function loadClases() {
    ...
    $('#marcadores-filtros-clase, #alumnos-filtros-clase').html('<option value="">Todos los cursos</option>');
    $.ajax({
        data: {usuario: usuario.id_usuario},
        url: self.SERVIDOR + "loadCursos.php",
        dataType: 'json',
        type:'POST',
        async: 'true',
        success:function(data) {
            for (var i = 0; i < data.cursos.length; i++) {
                ...
                $('#marcadores-filtros-clase, #alumnos-filtros-clase, #edit-marker-clase').append('<option value="'+curso.id_clase+'">'+curso.nombre+'</option>');
            }
...
function loadAlumnos() {
    ...
    $('#notas-filtros-alumno').html('<option value="">Todos los alumnos</option>');
    $.ajax({
        data: {usuario: usuario.id_usuario},
        url: self.SERVIDOR + "loadAlumnos.php",
        dataType: 'json',
        type:'POST',
        async: 'true',
        success:function(data) {
            for (var i = 0; i < data.alumnos.length; i++) {
                ...
                $('#notas-filtros-alumno').append('<option value="'+alumno.id_usuario+'">'+alumno.alumno+'</option>');
            }
...
function initEventsHandlers() {
    ...
    $('.menu-alumnos').on('click', function() { menu('alumnos'); });
