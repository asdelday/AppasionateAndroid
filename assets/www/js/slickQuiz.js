...
(function($){
    $.slickQuiz = function(element, options) {
        var plugin   = this,
            ...
            defaults = {
                checkAnswerText:  'Comprobar!',
                nextQuestionText: 'Siguiente &raquo;',
                backButtonText: '',
                tryAgainText: '',
                questionCountText: 'Pregunta %current de %total',
                preventUnansweredText: 'Debes seleccionar al menos una opción.',
                questionTemplateText:  '%count. %text',
                scoreTemplateText: '%score / %total',
                nameTemplateText:  '<span>Cuestionario: </span>%name',
                ...
                animationCallbacks: { // only for the methods that have jQuery animations offering callback
                	...
                	checkAnswer: function () {
                        var parametros = {"examen": examen, "score": myScore, "usuario": usuario.id_usuario};

                        $.ajax({
                            url: SERVIDOR + 'updateQuizScore.php',
                            type:'POST',
                            data:parametros,
                            dataType:'json',
                            beforeSend: function() {
                ...
                events: {
                	...
                	onCompleteQuiz: function (options) {
                        ...
                        var nota = (options.score * 10) / options.questionCount;
                        var notaRound = Math.round((nota * 100) / 100);
                        $('.quizLevel  strong').html("Nota: " + notaRound);
        ...
        plugin.method = {
                ...
                for (i in questions) {
                    if (questions.hasOwnProperty(i)) {
                        ...
                        if (plugin.config.displayQuestionCount) {
                            questionHTML.append('<div class="text-main ' + questionCountClass + '">' +
                                plugin.config.questionCountText
                                    .replace('%current', '<span style="font-size:17px;" class="current">' + count + '</span>')
                                    .replace('%total', '<span style="font-size:17px;" class="total">' +
                        ...
                        questionHTML.append('<h3 style="color: #888;">' + formatQuestion + '</h3>');
                        ...
                        if (plugin.config.perQuestionResponseMessaging || plugin.config.completionResponseMessaging) {
                            ...
                            responseHTML.append('<li style="color:#05CB22; font-weight: bold;" class="' + correctResponseClass + '"><i class="fa fa-check"></i> ' + question.correct + '</li>');
                            responseHTML.append('<li style="color:#f00; font-weight: bold;" class="' + incorrectResponseClass + '"><i class="fa fa-times"></i> ' + question.incorrect + '</li>');
                        ...
                        if (!plugin.config.perQuestionResponseMessaging) {
                            questionHTML.append('<a href="#" class="btn btn-main ' + nextQuestionClass + ' ' + checkAnswerClass + '">' + plugin.config.nextQuestionText + '</a>');
                        } else {
                            questionHTML.append('<a href="#" class="btn btn-main ' + nextQuestionClass + '">' + plugin.config.nextQuestionText + '</a>');
                            questionHTML.append('<a href="#" class="btn btn-success ' + checkAnswerClass + '"><i class="fa fa-check"></i> ' + plugin.config.checkAnswerText + '</a>');
                        }
            ...
            completeQuiz: function(options) {
                ...
                $('.rank-span').html();
                $('.rank-span').addClass('level' + levelRank);
